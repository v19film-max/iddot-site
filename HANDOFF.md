...
> **`CLAUDE.md`와 `AGENTS.md`는 이 파일(`HANDOFF.md`)로 향하는 심링크입니다.**
> Claude Code는 프로젝트 폴더를 열면 `CLAUDE.md`를, Codex는 `AGENTS.md`를 세션 시작 시
> 자동으로 읽습니다. 실제 내용은 이 파일 하나뿐이라 어느 도구를 쓰든 자동으로 같은 맥락을
> 파악하고, 사람이 내용을 복사해서 넘겨줄 필요가 없습니다. **수정할 때도 이 파일(HANDOFF.md)
> 하나만 고치면 됩니다** — 심링크라 다른 두 파일도 자동으로 같이 바뀝니다.
>
> **작업을 마칠 때마다 아래 "최근 작업 로그"에 짧게 한 줄만 남겨주세요.** 다음에 다른 도구가
> 이 폴더를 열었을 때 지난번에 어디까지 됐는지 바로 알 수 있게 하기 위함입니다. 최신 항목이
> 위로 오게 추가하세요.
...
- **2026-07-24 (Claude Code)** — `CLAUDE.md`/`AGENTS.md` 심링크 설정, 이 로그 섹션 추가.
  이 위로 새 항목을 계속 추가해나가면 됩니다.
